// Copyright (c) 2026, Aman Kumar and contributors For license information, please see license.txt Unity Games registration. `game_entry_details` is the multi-game cart (hidden in desk, shown only here). On picking a student its existing registrations are pre-filled (read-only); new games are added below.
// Consent is implied by submission (visible note + hidden consent=1).
// The server (Game Entry) re-validates everything, skips already- registered games, and fans the cart out into one record per game.
...
let feeSchedule = [];
let priorPaid = false;
	// Scope all v1 CSS to body.ug-form (set here so nothing leaks to other pages).
...
wf.set_value(fieldname, "");
	...
	// `frappe.web_form.on(fieldname, …)` does NOT reliably fire for Select controls in web forms — bind via event delegation on the static field wrapper so it survives the inner <select> being re-rendered.
...
renderTotalToPay();
	...
	// Sum of fees on NEW (non-locked) cart rows + GA base/late if no prior paid carrier exists.
...
if (r.is_late) late = Number(r.amount) || 0;
		...
		// If guardian already paid base for this season, server skips it; UI mirrors via priorPaid flag.
...
const f = cartField();
if (!f || !f.grid || !f.grid.wrapper) { setTimeout(startCartObserver, 500); return; }
const target = f.grid.wrapper.get(0);
if (!target || target._ugObs) return;
const obs = new MutationObserver(() => paintCartRows());
obs.observe(target, { childList: true, subtree: true });
target._ugObs = obs;
		// Block delete/check on locked (already-paid) rows; capture phase so we beat Frappe's handler.
...
ev.stopPropagation();
frappe.msgprint(__(
"This game is already registered and paid for. Cancel the registration from the desk if needed."
));
}
}, true);
}
setTimeout(startCartObserver, 800);
	...
	// Avatar removed by request — no-op kept so existing call sites stay safe.
...
if (!host) return;
if (!rows || !rows.length) { host.innerHTML = ""; host.style.display = "none"; return; }
...
setConsent("");
lastSentTo = null;
renderConsentArea && renderConsentArea("init");
if (!value) {
studentGender = "";
renderAvatar(null);
...
frappe.show_alert({
message: __(
"No age band is valid for this student in the selected game."
),
indicator: "orange",
});
}
...
wf.set_value("consent_otp", verifiedOtp);
		// Belt-and-braces: also poke the doc directly because set_value on a read_only Data field doesn't always persist.
if (frappe.web_form.doc) frappe.web_form.doc.consent_otp = verifiedOtp;
}

let lastSentTo = null; // {email_masked, mobile_masked}
let verifiedOtp = ""; // JS-side mirror — wf.set_value on a read_only
	                     // Data field is unreliable across Frappe versions.
...
renderConsentArea("failed");
	...
	// Consent area is initialised here and re-rendered by loadStudentSnapshot / onAuthorityChanged when either parent value changes.
